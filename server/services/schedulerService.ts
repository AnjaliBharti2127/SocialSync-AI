...
import { ActivityLog } from "../models/ActitvityLog.js";
        ...
        try{
            ...
            for( const post of postsToPublish){
                try{
                  ...
                  console.log(`Published post ${post._id} to Zernio with media: ${post.mediaUrl || "none"}`)
            ...
            if(postsToPublish.length > 0){
                console.log(`Evaluate ${postsToPublish.length} posts at ${now.toISOString()}`);
            }
        }catch(error){
            console.error("Error in schduled:,error");
        }
    })
    console.log("Scheduled service intialised.");
}